...
finish() {
  ...
  # 여기서 상태 파일에 손대면 안 됩니다.
  #   moh reload 는 새 껍데기를 띄우고 옛것을 죽입니다. 순서가 뒤바뀌면
  #   죽는 쪽이 살아 있는 쪽의 상태를 덮어써서, 멀쩡한 채널이 '멈춤' 으로
  #   보입니다. 실제로 재설치할 때마다 전 채널에 (준비안됨) 이 떴습니다.
  #   껍데기가 정말 죽었는지는 프로세스를 세어 보는 --status / verify.sh 가
  #   봅니다. 상태 파일은 살아 있는 쪽 것만 남깁니다.
  exit 0
}
