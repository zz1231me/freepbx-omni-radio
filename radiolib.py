...
#   off    아무것도 안 함
#   soft   (기본) 낮은 잡음을 걷어내고 리샘플링을 좋은 것으로.
#          귀로 티가 나라고 만든 게 아니라 '손해 안 보게' 하는 쪽입니다.
#          차이가 안 느껴지는 게 정상이고, 그게 이 설정의 목적입니다.
#   clear  음량을 고르게 하고 중고음을 올려 말을 또렷하게 만듭니다.
#          다만 실제로 들어 보면 조용한 구간의 잡음까지 같이 끌어올려서
...
def load(path: Path | None = None) -> tuple[list[dict], dict]:
    ...
    cfg = {
        ...
        "audio": str(data.get("audio", DEFAULT_AUDIO)).strip().lower(),
